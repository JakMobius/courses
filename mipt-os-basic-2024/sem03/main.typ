...
#focus-slide[
  ...
  #uncover((beginning: 2))[
    #text(size: 25pt)[
      #place(center + bottom)[
        #codebox(lang: "c", "fopen(...)") возвращает #codebox(lang: "c", "FILE*"), а не дескриптор
      ]
    ]
  ]
]
...
#slide(header: [Чтение и запись], background-image: none, place-location: horizon)[
  ...
  - Существуют #codebox(lang: "c", "pread(...)") и #codebox(lang: "c", "pwrite(...)") , которые *явно принимают позицию файла*.
...
#slide(header: [Какие флаги можно настраивать?], background-image: none)[
  ...
  #cetz.canvas(
    length: 1cm, {
      ...
      let background(color) = {
        box(fill: cell-color(base-color: color).background-color, width: 100%, height: 100%)
      }

      let title(color, content) = {
        place(top + left)[
          #box(inset: 15pt)[
            #text(weight: "black", size: 30pt, fill: cell-color(base-color: color).stroke-color)[
              #content
            ]
          ]
        ]
      ...
      cetz.draw.content((0, -9), (30, -16), padding: none, {
        background(black)
        title(black, [Диск])
...
#let largecell(color, content) = [
  ...
  #box(radius: 10pt, fill: color.background-color, stroke: 3pt + color.stroke-color, inset: 30pt)[
    #set text(fill: black)
    #content
  ]
]
...
#let draw-dir-ent(position, values, more: true) = {
  ...
  cetz.draw.content((left, position.y), (right, position.y - index-node-height), padding: none, {
    set align(horizon + center)
    let color = cell-color(base-color: black)
    box(width: 100%, height: 100%, fill: color.background-color, stroke: 2pt + color.stroke-color, radius: 10pt)[
      *Index Node*
    ]
  })

  cetz.draw.line((position.x, position.y - index-node-height - 0.1), (position.x, position.y - index-node-height - margin + 0.1), stroke: 4pt + black)

  cetz.draw.content((left, position.y - dirent-start), (right, position.y - dirent-start - dirent-height), padding: none, {
    set align(center)
    set block(spacing: 5pt)
    set text(size: 25pt)
    let colors = cell-color(base-color: byte-colors.at(1))

    grid(rows: (1.8cm,) * values.len(), columns: 100%, align: horizon + center, stroke: 2pt + colors.stroke-color, fill: colors.background-color, ..values)
    if(more) {
      text(size: 30pt)[*...*]
    }
  })
...
#slide(place-location: center + top, background-image: none)[
  #cetz.canvas(length: 1cm, {
      ...
      let largecodebox(lang: none, content) = {
        ...
        box(radius: 5pt, fill: color.background-color, width: 12cm, height: 1.7cm, stroke: 2pt + color.stroke-color)[
          #set align(horizon + center)
          #set text(fill: black)
          #set raw(theme: "../theme/halcyon.tmTheme")
          #raw(lang: lang, content)
      ...
      let data = (
          ...
          (`.`, `..`, [*`bin`*], `share`),
          (`.`, `..`, [*`bash`*],`mkdir`),
          ("0x7f", "0x45", "0x4c", "0x46").map(a => codebox(a)),
        )
      ...
      cetz.decorations.flat-brace(
        (index-node-positions.at(0) - 2.5, -8),
        (index-node-positions.at(0) - 2.5, -8 - 1.8 * 4), flip: true, name: "brace")
...
#slide(place-location: center + horizon)[
  ...
  #box(width: 65%, height: 10cm)[
    #align(center)[
      #set block(spacing: 40pt)
      #text(size: 60pt, weight: "black")[#largecell(byte-colors.at(1), [Directory Entry])]
    ]
...
#slide(header: [Что, если на Index Node несколько ссылок?], background-image: none)[
  #place(horizon + center)[
    #cetz.canvas(length: 1cm, {
  ...
  #place(center + bottom)[
    #uncover((beginning: 2))[
      #text(size: 25pt, [Это называется *жесткая ссылка.* Так можно только с файлами.])
    ]
  ]
]
...
#slide(header: [VFS - Обобщённый интерфейс для ФС.], place-location: horizon + center)[
  #set align(center)
...
#slide(header: [#codebox("procfs") -- не файловая файловая система.], place-location: horizon)[
  #set list(marker: none)

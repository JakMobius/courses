...
#slide(background-image: none)[
  ...
  #place(bottom, dy: -2cm, dx: 1cm)[
    #uncover((beginning: 2))[
      ...
      #box(inset: 20pt, baseline: 20pt, radius: 10pt, stroke: 3pt + green, fill: green.lighten(70%))[Решение -- мультиплексеры!]
    ]
  ]
]
  ...
  #place(center)[
    #box(width: 110%)[
      #table(
        ...
        [
          - Самый старый мультиплексер в Linux;
...
#slide(background-image: none)[
  ...
  #v(1cm)
  - Если к вашему серверу кто-то пытается подключиться, мультиплексер сообщит об этом так, как будто на сокете есть данные для чтения.
...
#slide(background-image: none)[
  ...
  - Создать мультиплексер и добавить в него дескриптор сокета.
  - Вызвать #codebox(lang: "c", "listen()") на сокете, и начать слушать подключения.
  - *#codebox(lang: "c", "while(true):")*
    - Ожидать события на мультиплексере.
    - Если событие на дескрипторе сокета, то доступно новое подключение. Нужно принять его (#codebox(lang: "c", "accept()")) и добавить в мультиплексер.
    - Если событие на дескрипторе подключения, то на нём появились данные, либо клиент отключился.
    - Если клиент отключился, то нужно закрыть соединение и удалить дескриптор из мультиплексера.
]

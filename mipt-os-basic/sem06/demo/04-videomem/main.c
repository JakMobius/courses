...
void initVulkan() {
    ...
    #ifdef __APPLE__
        // On macOS (MoltenVK) request the portability enumeration extension
        // and add it to the list of required GLFW extensions.
        uint32_t extCount = glfwExtensionCount + 1;
        const char** extNames = malloc(extCount * sizeof(const char*));
        for (uint32_t i = 0; i < glfwExtensionCount; ++i) {
            extNames[i] = glfwExtensions[i];
        }
        extNames[glfwExtensionCount] = VK_KHR_PORTABILITY_ENUMERATION_EXTENSION_NAME;
        glfwExtensions = extNames;
        glfwExtensionCount = extCount;
    #endif
    ...
    queueCreateInfo.pQueuePriorities = &queuePriority;
        ...
        VkDeviceCreateFlags createFlags = VK_KHR_portability_enumeration;
    #else
        const char* deviceExtensions[] = { VK_KHR_SWAPCHAIN_EXTENSION_NAME };
        VkDeviceCreateFlags createFlags = 0;
    #endif

    /* Note: after the VkDeviceCreateInfo is declared below, set:
       deviceCreateInfo.flags = createFlags;
    */

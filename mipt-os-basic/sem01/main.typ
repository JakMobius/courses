...
#slide(place-location: top)[
  ...
  #box(stroke: (left: black + 4pt), inset: (x: 15pt, y: 15pt))[
    ...
    #box(baseline: 7pt, image("img/mail.png", width: 30pt))
      #link("mailto:klimov.aiu@phystech.edu")[*klimov.aiu\@phystech.edu*]
...
#slide(header: [Что делает нас программистами], place-location: horizon)[
  ...
  - #bash("git"): система контроля версий.
]
...
#slide(
  ...
  - Если #codebox("0x7f 0x45 0x4c 0x46"), то запускает его как ELF-файл;

  - Если #codebox("#!/usr/bin/python3"), то использует указанный интерпретатор;

  - Иначе файл запускается как шелл-скрипт.
]
...
#slide(
  header: [Как пользоваться gdb], background-image: none,
)[
  #v(10pt)
  #set par(spacing: 14pt)

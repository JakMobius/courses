...
#let float-value(value) = {
  if(float-is-zero(value)) {
    return if value.sign { -0 } else { +0 }
  }

  if(float-is-infinity(value)) {
    return if value.sign { -float.inf } else { +float.inf }
  }

  if(float-is-nan(value)) {
    return if value.sign { -float.nan } else { +float.nan }
  }
  
  let factor-power = value.exponent - int.bit-lshift(1, value.exponent-bits - 1) + 1

  if not float-is-normalized(value) {
    factor-power += 1
  }
  
  let result = float-get-full-mantissa(value) * calc.pow(2, factor-power - value.mantissa-bits)
...
#let float-bits(value) = {
  let bit-array = (value.sign,)

  for i in array.range(value.exponent-bits) {
    if is-bit-set(value.exponent, value.exponent-bits - i - 1) {
      bit-array.push(true)
    } else {
  ...
  for i in array.range(value.mantissa-bits) {
    let bit = is-bit-set(value.mantissa, value.mantissa-bits - i - 1)
    bit-array.push(bit)
  }
